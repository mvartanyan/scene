...
- **Runs**: Launch runs (baseline/comparison), tweak the timeout (seconds) before submitting, and inspect executions via the modal. The run log and modal are powered by HTMX and the vendored `app/static/htmx.min.js`.
